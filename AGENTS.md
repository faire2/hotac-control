...
- **React 16.14 stays pinned.** A React 18 bump is a separate, scoped phase (see ROADMAP backlog).
- **Bootstrap / react-bootstrap pin is lifted** (2026-05-11). `bootstrap@^4.4.1` and `react-bootstrap@^1.0.0-beta.16` are still installed and used by existing components, but new work may freely replace, remove, or restyle around them. New squad-card visual pass (`Squad.cockpit.css`) demonstrates the target direction: pure-CSS classes-in-stylesheet, CSS variables for theming, optional react-bootstrap usage. Eventually those deps will be removed entirely.
- **Styling approach for new UI:** plain CSS / CSS Modules / SCSS in `.css` files next to the component. No inline `style={{...}}` props except for genuinely dynamic computed values. No CSS-in-JS / styled-components / emotion. CSS variables (e.g. `--accent-holo`, `--surface`) carry the palette so alt themes (Terminal green) can swap by overriding the same names on a parent.
